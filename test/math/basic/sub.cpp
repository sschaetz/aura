#define BOOST_TEST_MODULE math.sub
...
BOOST_AUTO_TEST_CASE(sub_float)
{
	initialize();
	...
	for (auto x : sizes) {
		...
		math::sub(device_input1, device_input2, device_output, f);
...
BOOST_AUTO_TEST_CASE(sub_cfloat)
{
	initialize();
	int num = device_get_count();
	BOOST_REQUIRE(num > 0);
...
BOOST_AUTO_TEST_CASE(sub_float_cfloat)
{
	initialize();
	int num = device_get_count();
	BOOST_REQUIRE(num > 0);
...
BOOST_AUTO_TEST_CASE(sub_cfloat_float)
{
	initialize();
	int num = device_get_count();
	BOOST_REQUIRE(num > 0);
