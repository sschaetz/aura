#ifndef AURA_MATH_BASIC_ADD_HPP
#define AURA_MATH_BASIC_ADD_HPP
...
{
	return std::make_tuple("add_float",
			R"aura_kernel(
	
	#include <boost/aura/backend.hpp>

	AURA_KERNEL void add_float(AURA_GLOBAL float* src1,
...
inline std::tuple<const char*,const char*> get_add_kernel(cfloat, cfloat, cfloat)
{
...
inline std::tuple<const char*,const char*> get_add_kernel(float, cfloat, cfloat)
{
	return std::make_tuple("add_cfloat",
			R"aura_kernel(
...
inline std::tuple<const char*,const char*> get_add_kernel(cfloat, float, cfloat)
{
	return std::make_tuple("add_cfloat",
			R"aura_kernel(
...
void add(const DeviceRangeType1& input_range1,
