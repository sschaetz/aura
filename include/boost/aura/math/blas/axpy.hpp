#ifndef AURA_MATH_BLAS_AXPY_HPP
#define AURA_MATH_BLAS_AXPY_HPP
...
inline std::tuple<const char*,const char*> get_axpy_kernel(float, float, float)
{
	return std::make_tuple("axpy_float",
			R"aura_kernel(

	#include <boost/aura/backend.hpp>

	AURA_KERNEL void axpy_float(AURA_GLOBAL float* src1,
...
inline std::tuple<const char*,const char*> get_axpy_kernel(cfloat, cfloat, cfloat)
{
	return std::make_tuple("axpy_cfloat",
			R"aura_kernel(

	#include <boost/aura/backend.hpp>

	AURA_KERNEL void axpy_cfloat(AURA_GLOBAL cfloat* src1,
			...
			unsigned long N)
	{
		unsigned int i = get_mesh_id();
		if (i < N) {
                       src3[i] = caddf(cmulf(src1[0],src2[i]) , src3[i]);
		}
	}
...
void axpy(const DeviceRangeType& input_range1,
		const DeviceRangeType& input_range2,
		DeviceRangeType& input_output_range, feed& f)
{
	// asserts to make sure the first argument has size 1.
        assert(1 == aura::traits::size(input_range1));
	...
	auto kernel_data = detail::get_axpy_kernel(
