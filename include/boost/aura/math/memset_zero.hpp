#ifndef AURA_MATH_MEMSET_ZERO_HPP
#define AURA_MATH_MEMSET_ZERO_HPP
...
inline std::tuple<const char*,const char*> memset_zero_kernel_name(
                float dst_ptr)
{
        return std::make_tuple("memset_zero_float",
            R"aura_kernel(
...
inline std::tuple<const char*,const char*> memset_zero_kernel_name(
                cfloat dst_ptr)
{
        return std::make_tuple("memset_zero_cfloat",
            R"aura_kernel(

        #include <boost/aura/backend.hpp>

        AURA_KERNEL void memset_zero_cfloat(AURA_GLOBAL cfloat* dst_ptr)
        {
            unsigned long id = get_mesh_id();
                dst_ptr[id] = make_cfloat(0.,0.);
        }
...
void memset_zero(DeviceRangeType& output_range, feed& f)
{
        // get suitable kernel string for the ouput value type
        auto kernel_data = detail::memset_zero_kernel_name(
...
#endif // AURA_MATH_MEMSET_ZERO_HPP
