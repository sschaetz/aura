...
void copy(const device_ptr<T>& first, const device_ptr<T>& last, OutputIt dst_first, feed& f)
{
        f.get_device().activate();
        AURA_CUDA_SAFE_CALL(
                cuMemcpyDtoHAsync(
...
template <typename T>
void copy(const device_ptr<T>& first, const device_ptr<T>& last, device_ptr<T>& dst_first, feed& f)
{
        f.get_device().activate();
        AURA_CUDA_SAFE_CALL(
                cuMemcpyDtoDAsync(
