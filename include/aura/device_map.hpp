...
{
	device_map<T> r;
	r.ptr_ = ::aura::backend::device_map(addressof(c), size(c), tag, d);
	r.hptr_ = addressof(c);
	r.size_ = size(c);
...
public:
	...
	device_map(C& c, memory_tag tag, device& d) : 
		dptr_(::aura::backend::device_map_alloc(addressof(c), 
					::aura::size(c), tag, d)),
		hptr_(addressof(c)),
		size_(::aura::size(c)),
	...
	device_map(C& start, C& end, memory_tag tag, device& d) : 
		dptr_(::aura::backend::device_map_alloc(addressof(start), 
	...
	~device_map() 
	{
		::aura::backend::device_map_free(hptr_, dptr_);	
	}
	...
	{
		assert(::aura::addressof(c) == hptr_);
		assert(::aura::size(c) == size_);
