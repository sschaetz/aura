                       ...
                       std::size_t params)
{
	...
	case 7:
		...
		break;
	case 8:
		invoke(k, mesh, bundle, args(f1, f2, f3, f4, 
					f5, f6, f7, f8), f);
		break;
	case 9:
		invoke(k, mesh, bundle, args(f1, f2, f3, f4, 
        ...
        std::bitset< sizeof(ops_tbl)/sizeof(ops_tbl[0]) > & ops)
{
	...
	if(ops[4]) { // ctx
		...
		AURA_BENCHMARK(run_ctx(dev_ordinal), runtime, 
				min, max, mean, stdev, runs);
		print_benchmark_results(ops_tbl[4], 
				min, max, mean, stdev, runs, runtime);
	}

	if(ops[5]) { // ctxfeed
		run_ctxfeed(dev_ordinal); // dry run
		AURA_BENCHMARK(run_ctxfeed(dev_ordinal), runtime, 
				min, max, mean, stdev, runs);
		print_benchmark_results(ops_tbl[5], 
				min, max, mean, stdev, runs, runtime);
	}
	...
	// benchmarks requiring device and feed
	{
		...
		if(ops[0]) { // sync
			run_sync(f); // dry run
			AURA_BENCHMARK(run_sync(f), runtime, 
					min, max, mean, stdev, runs);
			print_benchmark_results(ops_tbl[0], 
					min, max, mean, stdev, runs, runtime);
		}
		if(ops[1]) { // synck
			kernel nak = create_kernel(m, "kernel_0arg");
			run_synck(f, nak); // dry run
			AURA_BENCHMARK(run_synck(f, nak), runtime, 
					min, max, mean, stdev, runs);
			print_benchmark_results(ops_tbl[1], 
					min, max, mean, stdev, runs, runtime);
		}
		if(ops[2]) { // kern
			kernel nak = create_kernel(m, "kernel_0arg");
			run_kern(f, nak); // dry run
			AURA_BENCHMARK(run_kern(f, nak), runtime, 
					min, max, mean, stdev, runs);
			print_benchmark_results(ops_tbl[2], 
					min, max, mean, stdev, runs, runtime);
...
int main(int argc, char *argv[])
{
	...
	while ((opt = getopt(argc, argv, "m:b:p:d:t:")) != -1) {
		...
		case 'p': {
			...
			params = boost::aura::generate_sequence<std::size_t, 
			       1>(optarg);
			break;
		}
		case 'd': {
		...
		default: {
			fprintf(stderr, "Usage: %s -m <meshsize> "
					"-b <bundlesize> -p <params> "
					"-d <device ordinal> "
					"-t <runtime (ms)> <operations>\n", 
					argv[0]);
