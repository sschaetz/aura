...
extern "C" __global__ void kernel_c2i(int count, float* dst, float* src1, float* src2)
{
...
extern "C" __global__ void kernel_i2c(int count, float* dst1, float* dst2, float* src)
{
	unsigned int i = get_mesh_id(); 
	if (i < count) {
		dst1[i] = src[2*i];
